...
// Solo la junta puede ver actas
if (!isset($_SESSION['rol']) || $_SESSION['rol'] != 'junta') {
    header("Location: index.php");
    exit;
}

// 🔴 ACTUALIZAR LA ÚLTIMA VISITA DEL USUARIO A ACTAS
$_SESSION['ultima_visita_actas'] = time();

$superadmin_email = 'admin@club.com';
$es_superadmin = (isset($_SESSION['usuario_email']) && $_SESSION['usuario_email'] == $superadmin_email);

$sql = "SELECT * FROM actas ORDER BY fecha_reunion DESC";
...
<head>
    ...
    <title>Actas - Ratas del Queiles</title>
    ...
    <style>
        ...
        .grid-actas { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 20px; margin-top: 20px; }
        .card-acta { background: #1a1a1a; border: 1px solid rgba(255,255,255,0.05); border-radius: 8px; padding: 16px; transition: border-color 0.2s; }
        .card-acta:hover { border-color: rgba(255,255,255,0.15); }
        .card-acta h3 { font-family: 'Anybody', sans-serif; font-size: 1.1rem; font-weight: 700; color: #e5e2e1; margin-bottom: 6px; }
        .card-acta .fecha { color: #888; font-size: 0.85rem; margin-bottom: 4px; }
        .card-acta .firmas { display: inline-block; background: #2a2a2a; padding: 2px 10px; border-radius: 12px; font-size: 0.75rem; color: #ffb59e; margin-bottom: 10px; }
        .card-acta .acciones { display: flex; gap: 8px; flex-wrap: wrap; margin-top: 12px; }
        .btn-pdf { background: #dc3545; color: white; padding: 4px 12px; border-radius: 4px; text-decoration: none; font-size: 0.75rem; font-weight: 600; }
        .btn-pdf:hover { background: #b02a37; }
        .btn-editar { background: #ffc107; color: black; padding: 4px 12px; border-radius: 4px; text-decoration: none; font-size: 0.75rem; font-weight: 600; }
        .btn-editar:hover { background: #e0a800; }
        .btn-borrar { background: #dc3545; color: white; padding: 4px 12px; border-radius: 4px; text-decoration: none; font-size: 0.75rem; font-weight: 600; }
        .btn-borrar:hover { background: #c82333; }
        .boton-nuevo { background: #28a745; color: white; padding: 8px 16px; border-radius: 4px; text-decoration: none; font-weight: 600; display: inline-block; }
...
<aside class="hidden md:flex flex-col fixed left-0 top-16 bottom-0 w-64 bg-surface-container border-r border-outline-variant p-4 z-40">
    <nav class="flex flex-col gap-2 mt-4">
        ...
        <a class="flex items-center gap-3 text-primary-container bg-on-primary-container/10 rounded-lg p-3" href="actas.php"><span class="material-symbols-outlined" style="font-variation-settings:'FILL'1;">description</span> Actas</a>
...
<main class="flex-grow p-margin-mobile flex flex-col gap-gutter pb-24 md:pb-8 max-w-container-max mx-auto w-full">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-outline-variant pb-4">
        <div>
            <h2 class="font-headline-lg text-headline-lg text-on-background uppercase tracking-tight">Registro de Actas</h2>
            <p class="text-on-surface-variant max-w-xl font-label-md text-label-md uppercase">Historial oficial de asambleas, acuerdos y resoluciones del club.</p>
        </div>
        <div class="mt-4 md:mt-0 flex gap-2">
            <?php if($es_superadmin): ?>
                <a href="nueva_acta.php" class="boton-nuevo">➕ Nueva Acta</a>
            <?php endif; ?>
        </div>
    </div>
                ...
                <div class="card-acta">
                    ...
                    <div class="acciones">
                        ...
                        <?php if ($es_superadmin): ?>
                            ...
                            <a href="borrar_acta.php?id=<?php echo $fila['id']; ?>" class="btn-borrar" onclick="return confirm('¿Borrar esta acta?')">🗑️ Borrar</a>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <p class="text-secondary">No hay actas registradas todavía.</p>
        <?php endif; ?>
    </div>
</main>
...
<nav class="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-unit pb-safe h-20 bg-surface-container border-t border-outline-variant">
    ...
    <a class="flex flex-col items-center justify-center text-primary-container bg-on-primary-container/10 rounded-xl p-1" href="actas.php"><span class="material-symbols-outlined" style="font-variation-settings:'FILL'1;">description</span><span class="font-label-sm uppercase text-xs">Actas</span></a>
    <?php if(isset($_SESSION['rol']) && $_SESSION['rol'] == 'junta'): ?>
